...
void multichoice_repel_box()
{
    ...
    if (no_of_options > 1)
    {
        ...
        for (u8 i = 0; i < no_of_options; i++)
            width = str_font1_get_min_width(RepelTexts[i].text_ptr, width);
        width = str_rbox_max_width(width);
        u8 x_pos = REPEL_X;
        x_pos = str_rbox_X_pos(x_pos, width);
        u8 rboxID = rbox_create_and_sth(x_pos, REPEL_Y, width, no_of_options * 2);
        rbox_frame_menupalette(rboxID, 0);
        rbox_put_multichoice_text(rboxID, no_of_options, &RepelTexts[0]);
