...
void* unsigned_int_to_string(u8* destination, u32 num, u8 formatting, u8 max_digits);
u8 pokesummary_create_rbox(const struct rbox_rom*, u8 index);
...
void clear_f7_texts(void);
void store_f7_text(u8 ID, const u8* string_ptr);
void display_f7_texts(const u8* dst, const u8* src);
...
u32 oe_read_word(void* ptr);
u16 read_hword(void* ptr);
...
void* strcpy_xFF_terminated_0(const u8* dst, const u8* src);
void* str_append(const u8* dst, const u8* src);
s8 compare_two_strings(const u8* string1, const u8* string2);
void sub_81972C4(u8 arg);
void textbox_load_box_graphics(void);
void fdecode_and_copy(const u8* dst, const u8* src);
u16 box_related_one(u8 arg0, u8 arg1, const u8* text, u8 speed, u32 sp_0, u32 sp_4, u32 sp_8, u32 sp_C);
u8 get_text_speed(void);
u8 task_add(void* function, u8 priority);
void task_add_textbox(void);
void rbox_prepare_textbox(u8 rboxID, u8 arg1);
u16 textbox_print_ret(void);
...
void fame_hall_pc_put_text(const u8* txt, const u8* text_toadd, u8 arg2, u8 arg3, bool to_vram);
...
u8 npc_pal_idx_for_given_tag(u16 npcID);
u8 npc_load_two_palettes__and_record(u16 npcID, u16 dstID);
void npc_paltag_set_load_on_warp(bool unkown);
void npc_pal_op(struct npc_state*, struct object*);
struct npc_type* get_npc_type_ptr(u8 sprite);
u8 gpu_pal_tags_index_of(u16 index);
void sub_80ABE18(u8 palID);

void gpu_pal_apply(const struct palette* src, u16 index, u16 num_colours);
